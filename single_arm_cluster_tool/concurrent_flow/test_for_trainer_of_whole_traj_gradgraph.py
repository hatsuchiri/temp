...
class GradientTracker:
    def __init__(self, logger=None):
        self.logger = logger if logger else logging.getLogger('gradient_tracker')
        self.tracked_tensors = {}
    
    def track(self, tensor, name):
        """Track a tensor's gradient information"""
    ...
    def log_all(self, step_name):
        """Log all tracked tensors' information"""
        self.logger.info(f"\n=== Gradient Tracking at {step_name} ===")
        for name, info in self.tracked_tensors.items():
            self.logger.info(f"Tensor: {name}")
            self.logger.info(f"  Shape: {info['shape']}")
            self.logger.info(f"  Requires Grad: {info['requires_grad']}")
            self.logger.info(f"  Grad FN: {info['grad_fn']}")
            if info['tensor'].grad is not None:
                self.logger.info(f"  Gradient: {info['tensor'].grad}")
            else:
                self.logger.info(f"  Gradient: None")
        self.logger.info(f"=== End Tracking at {step_name} ===")
...
def parse_arguments():
    ...
    parser.add_argument('--baselineN', type=int, default=32, help='Sample number for each baseline')
...
class Trainer:
    def __init__(self, env_params, model_params, trainer_params):
        ...
        self.logger1 = logging.getLogger('check_grad')
        self.grad_tracker = GradientTracker(self.logger1)

    def set_logger(self):
        self.logger1.setLevel(logging.DEBUG)
        hd1 = logging.FileHandler(f'{self.trainer_params["model_save"]["path"]}/check_grad.log', mode='a', encoding='utf-8')
        hd1.setFormatter(logging.Formatter('%(asctime)s - %(name)s - %(levelname)s - %(message)s'))
        hd1.setLevel(logging.DEBUG)
        self.logger1.addHandler(hd1)
    ...
    def train(self):
        ...
        for episode in range(self.trainer_params['num_episodes']):
            ...
            self.grad_tracker.clear()
            
            # Track initial state
            self.grad_tracker.track(batch_state.clock, 'batch_state.clock')
            self.grad_tracker.track(batch_state.loc_process_end_time, 'batch_state.loc_process_end_time')
            
            # Encoding phase
            self.model.encoding(batch_state)
            
            # Track encoded embeddings
            if hasattr(self.model, 'row_embed') and hasattr(self.model, 'col_embed'):
                self.grad_tracker.track(self.model.row_embed, 'model.row_embed')
                self.grad_tracker.track(self.model.col_embed, 'model.col_embed')
            
            self.grad_tracker.log_all('After Encoding')
            ...
            while not batch_state.done.all():                
                ...
                # Track action and probability tensors
                self.grad_tracker.track(batch_action, 'batch_action')
                self.grad_tracker.track(batch_prob, 'batch_prob')
            ...
            for _ in range(self.baselineN):
                ...
                while not batch_bstate.done.all():
                    ...
                    batch_next_bstate = _stack_states(next_bstates)
                    batch_next_bstate.batch_idx = torch.arange(batch_next_bstate.batch_size())
                    batch_next_bstate.to(self.device)  

                    batch_bstate = copy.deepcopy(batch_next_bstate)
                Nsumbmakespans = np.array(Nsumbmakespans) + np.array(bmakespans)/self.baselineN
            Avgbmakespans = torch.tensor(Nsumbmakespans)
            
            for makespan,Avgbmakespan in zip(makespans,Avgbmakespans):
                advantage = (-makespan)-(-Avgbmakespan)
                batch_advantage.append(advantage)    

            batch_prob_log_sum = torch.zeros((batch_state.batch_size()),dtype=torch.float32).to(self.device)
            for batch_prob in batch_prob_trajectory:
                batch_prob_log = torch.log(batch_prob).squeeze()
                batch_prob_log_sum += batch_prob_log


            print('start to calculate loss')
            self.grad_tracker.clear()
            
            # Track probability log sum and advantage
            self.grad_tracker.track(batch_prob_log_sum, 'batch_prob_log_sum')
            self.grad_tracker.track(torch.tensor(batch_advantage, device=self.device), 'batch_advantage')
            
            batch_loss = []
            for advantage,problog in zip(batch_advantage,batch_prob_log_sum):
                loss = -advantage * problog
                batch_loss.append(loss)   
            batch_loss = tuple(batch_loss)
            batch_loss = torch.stack(batch_loss)
            
            # Track loss before backward
            self.grad_tracker.track(batch_loss, 'batch_loss_before_backward')
            self.grad_tracker.log_all('Before Backward')

            self.optimizer.zero_grad()
            batch_loss.backward(torch.ones_like(batch_loss))
            
            # Track loss after backward
            self.grad_tracker.track(batch_loss, 'batch_loss_after_backward')
            
            # Track key model parameters' gradients
            if hasattr(self.model, 'encoder') and hasattr(self.model.encoder, 'layers'):
                for i, layer in enumerate(self.model.encoder.layers):
                    if hasattr(layer, 'row_encoding_block') and hasattr(layer.row_encoding_block, 'Wq'):
                        self.grad_tracker.track(layer.row_encoding_block.Wq.weight, f'encoder.layer{i}.row_encoding_block.Wq.weight')
            
            if hasattr(self.model, 'decoder') and hasattr(self.model.decoder, 'Wq_3'):
                self.grad_tracker.track(self.model.decoder.Wq_3.weight, 'decoder.Wq_3.weight')
            
            self.grad_tracker.log_all('After Backward')
            print('Start logging the gradient of each parameter into file')
            self.logger1.info(f'Start logging para in episode{episode}')
            for n,p in self.model.named_parameters():
                self.logger1.info(f'参数名：{n}')
                self.logger1.info(f'参数形状：{p.shape}')
                self.logger1.info(f'参数是否需要梯度：{p.requires_grad}')
                self.logger1.info(f'参数值：{p.grad}')
            self.logger1.info(f'End logging para in episode{episode}')
            
            self.logger1.info(f'grad_fn: {batch_loss.grad_fn}')

            self.optimizer.step()
            print(f"Episode {episode} end, Loss: {batch_loss}")   
