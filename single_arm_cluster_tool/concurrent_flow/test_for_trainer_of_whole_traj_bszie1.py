...
class Trainer:
    ...
    def train(self):
        ...
        for episode in range(self.trainer_params['num_episodes']):
            ...
            batch_prob_log_sum = torch.empty(batch_state.batch_size()).to(self.device)
            ...
            print('模型参数梯度',self.model.parameters())
            self.optimizer.step()
            print(f"Episode {episode}, Loss: {batch_loss}")   
