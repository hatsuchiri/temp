...
class Trainer:
    ...
    def train(self):
        ...
        for episode in range(self.trainer_params['num_episodes']):
            ...
            # Initialize batch
            for b in range(self.trainer_params['batch_size']):
                state = envs[b].reset_except_recipes()
            ...
            self.logger.info('start to calculate loss')
            loss_trajectory = []
            for makespan,prob in zip(makespans,batch_prob_trajectory):
                loss = -(-makespan) * torch.log(prob)
            ...
            #     print(_.grad)
            self.optimizer.step()
            print(f"Episode {episode}, avgLoss: {sum_loss.mean()}")
            print(f'avgMakespan: {sum(makespans)/batch_state.batch_size()}')   
            self.logger.info(f"Episode {episode}, avgLoss: {sum_loss.mean()}")
            self.logger.info(f'avgMakespan: {sum(makespans)/batch_state.batch_size()}')   
    ...
    def set_logger(self):
        ...
        filehandler = logging.FileHandler(f"{self.trainer_params['model_save']['path']}/log.txt")
    ...
    def test_after_train(self):
        ...
        envs = self.envs
        states = [None for _ in range(self.trainer_params['batch_size'])]
        ...
        # collect results
        for instance_id in range(self.trainer_params['batch_size']):
...
def main():
    ...
    print("="*60)
    trainer.logger.info(f'Average makespan of CBS: {df["CBS Makespan"].mean():.2f}, RL: {df["RL Makespan"].mean():.2f}')
    pd.set_option('display.max_rows', None)  
    pd.set_option('display.max_columns', None) 
    pd.set_option('display.width', None)  
    pd.set_option('display.max_colwidth', None)  
    print(df)
    trainer.logger.info(df)
